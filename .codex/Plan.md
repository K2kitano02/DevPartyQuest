...
最後の質問に回答したあと、最もスコアが高いタイプが1つなら、そのタイプを診断結果として表示します。

### 同点処理

最高点のタイプが複数ある場合は、結果ページの前に `FINAL QUEST` を表示します。

- 8タイプごとに、そのタイプらしい行動を表す最終選択文を1つ定義します。
- 同点になったタイプの選択肢だけを表示します。
- 2タイプ同点なら2択、3タイプ以上の同点なら同点タイプ数と同じ数の選択肢を表示します。
- 選択肢には診断タイプ名を表示せず、行動文だけを表示します。
- ユーザーが選んだタイプを、そのまま最終結果として使用します。
- 組み合わせごとの専用問題や、選択後のスコア加算は行いません。
...
- 回答を選ぶとスコアを加算します。
- 回答後、自動で次の質問へ進みます。
- 8問目の回答後、最高点が1タイプなら結果ページを表示します。
- 最高点が複数タイプなら `FINAL QUEST` を表示します。
- 2問目以降は「前の質問に戻る」で直前の質問と回答前のスコアへ戻れます。
...
- 同点タイプの選択肢だけを表示します。
- 選択したタイプを診断結果として結果ページへ進みます。
- 診断タイプ名は選択前に表示しません。
- 「前の質問に戻る」で8問目と、その回答前のスコアへ戻ります。
...
```ts
type AppStep = "start" | "question" | "tieBreak" | "result";
```
...
- 最高点が複数ある場合は `FINAL QUEST` で最終結果を選べる
- 結果画面にタイプ詳細が表示される
- もう一度診断できる
- Xで診断結果を共有できる

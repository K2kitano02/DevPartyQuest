...
- npm test
- npm run build
- Playwright MCP で単独1位が直接結果画面へ進むことを確認
- Playwright MCP で2タイプ同点が2択、3タイプ同点が3択になることを確認
- Playwright MCP で選択前にロール名が表示されず、選択したロールの結果へ進むことを確認
- Playwright MCP で `FINAL QUEST` から8問目へ戻り、同じ回答で再び同点になることを確認
- Playwright MCP で8問目を別回答へ変更すると、復元後のスコアから結果が再判定されることを確認
- Playwright MCP で390px幅に横スクロールがないことを確認
- Playwright MCP でコンソールエラー・警告なしを確認

# なぜ必要か

- 固定の優先順位だけで同点結果を決めず、ユーザーがより自分に近い行動を選べるようにするため。
- 2タイプだけでなく、3タイプ以上の同点にも同じ仕組みで対応するため。

# 必要なこと(簡易的に)

- 最高点のタイプをすべて取得する。
- 最高点が1タイプなら従来どおり結果画面へ進む。
- 最高点が複数タイプなら `FINAL QUEST` を表示する。
- 同点タイプに対応する行動文だけを選択肢として表示する。
- 選択したタイプを最終結果として結果画面へ進む。
- `FINAL QUEST` から8問目の回答前へ戻れるようにする。

# やること(コードレベル)

- 8タイプ分の最終選択文をデータとして定義する。
- 複数の最高点タイプを返す診断ロジックを追加する。
- `FINAL QUEST` 用のコンポーネントを追加する。
- `AppStep` と `App.tsx` に同点選択画面の状態を追加する。
- 戻る場合は8問目を再表示し、`scoreHistory` から回答前のスコアを復元する。
- 診断開始時と再診断時に同点タイプの状態を初期化する。
- 2タイプ同点、3タイプ以上の同点、単独1位、選択後の結果遷移をテストする。

# ゴール(期待する動作など)

- 単独1位の場合は追加画面を表示せず結果へ進む。
- 同点の場合だけ `FINAL QUEST` が表示される。
- 同点ではないタイプの選択肢は表示されない。
- 表示された選択肢から1つ選ぶと、対応する診断結果が表示される。
- 「前の質問に戻る」で8問目へ戻り、回答を選び直せる。
- デスクトップ幅と390px幅で操作でき、コンソールエラーがない。
